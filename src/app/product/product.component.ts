...
export class ProductComponent implements OnInit {
  ...
  getFirstIndexOfId() {
    /*    // let firstCharOfIds: string[] = [];
        // firstCharOfIds.push( this.bundles.filter(value => value.id).toLocaleString() )
        // console.log(this.bundles.filter(value => value.id))
        // let isItWorking = this.bundles.map(value => value.id.slice(0, 1)).reduce(function (acc: any, curr: any) {
        //   return acc[curr] ? ++acc[curr] : acc[curr] = 1, acc
        // }, {})
        // console.log(isItWorking)
        // return isItWorking;
        // IT IS WORKING*/
    return this.bundles.map(value => value.id.slice(0, 1)).reduce(function (acc: any, curr: any) {
      return acc[curr] ? ++acc[curr] : acc[curr] = 1, acc
    }, {});
  }
  ...
  /*  // getFilteredId(): any {
    //   let arrSize = new Array<number>(9);
    //   let myArr: string[] = [];
    //
    //   for (let i = 1; i <= 9; i++) {
    //     console.log("i",i)
    //   }
    //
    //   // console.log("asdasasd", myArr)
    //   return myArr;
    // }
    // getStringArrayOfIds(): string[] {
    //   // this.bundles.filter()
    //
    //   this.bundleIdArray.push();
    //   return this.bundleIdArray;
    // }
    /!*  getFilteredId(numberToSearch: number):string[] {
        let bundleIdArray: string[] = [];
        // let size = new Array<number>(numberToSearch);

        // console.log("boyut",size);
        // for (const bundle of this.bundles) {
        //   this.bundleIdArray.push(bundle.id.toString().slice(0,1))
        // }
        // console.log("hmm", bundleIdArray)
        return bundleIdArray;
        /!*    this.bundleIdArray.push(this.bundles
              .map(
                (productCursor) => {
                  productCursor.id.toLocaleString().slice(0, 1) === Number(numberToSearch).toLocaleString();
                  numberToSearch--;
                }
              )
              .length.toLocaleString())
            console.log("hmm", this.bundleIdArray)
            return this.bundleIdArray;*!/
      }*!/
    /!*  getFilteredId(): any {
      for (let i = 0; i < this.bundles.length; i++) {
        ...
        return this.bundles.filter(
          (productCursor) => {
            productCursor.id.toLocaleString().slice(0, 1) === Number().toLocaleString();
            console.log(productCursor.id);
          }
        ).length.toLocaleString();
      }*!/
    ...
    // results: any[] = [];
    // ids: any;
    // getFilteredId(): string[] {
    //   return this.bundles.map(value => value.id.startsWith("1",0));
    ...
    //   // }
    //
    //   // for (let bundlesKey in this.bundles) {
    //   //   return
    //   // }
    // }
    ...
    /!*  this.bundles.map(item => {
      ...
      // !!! TODO !!! iç içe iki döngü olacak ilkinde bundle id leri arasında ikincisinde ise 1 den 9 a kadarki numaralar arasında !!!
    /!*  return this.bundles.filter(
        (productCursor) =>{

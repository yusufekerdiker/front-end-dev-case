...
export class ProductFilterPipe implements PipeTransform {
  transform(value: Product[], enteredSearchValue: string): Product[] {
    let query = value
      ? value.filter(
          (val: Product) =>
            val.name.toLocaleLowerCase().indexOf(enteredSearchValue) !== -1 ||
            val.bundle.toLocaleLowerCase().indexOf(enteredSearchValue) !== -1 ||
            val.studio.toLocaleLowerCase().indexOf(enteredSearchValue) !== -1
        )
      : value;
